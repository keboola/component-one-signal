...
class NotificationsWriter(ResultWriter):
    ...
    def __init__(self, out_path, buffer=8192):
        ...
        notifications_data_tbl = KBCTableDef('notification_data', [], ['notification_id'])
        self.data_writer = ResultWriter(out_path, notifications_data_tbl,
                                        exclude_fields=['data'],
                                        flatten_objects=False,
                                        user_value_cols={'notification_id'}, buffer_size=buffer)

        notifications_filter_tbl = KBCTableDef('notification_filters', [], ['notification_id'])
